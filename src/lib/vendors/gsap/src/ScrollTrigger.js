...
let gsap,
	...
	_clearScrollMemory = () =>
		_scrollers.cache++ && _scrollers.forEach((obj) => typeof obj === 'function' && (obj.rec = 0)), // zero-out all the recorded scroll positions. Don't use _triggers because if, for example, .matchMedia() is used to create some ScrollTriggers and then the user resizes and it removes ALL ScrollTriggers, and then go back to a size where there are ScrollTriggers, it would have kept the position(s) saved from the initial state.
	_refreshingAll,
	_refreshID = 0,
	...
	init(vars, animation) {
		...
		if (snap) {
			...
			snapFunc = _isFunction(snap.snapTo)
				...
				? (value, st) =>
						_snapDirectional(snap.snapTo)(value, _getTime() - lastRefresh < 500 ? 0 : st.direction)
