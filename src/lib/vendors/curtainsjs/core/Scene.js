...
export class Scene {
	...
	addToRenderTargetsStack(plane) {
		...
		const renderTargetsPlanes = this.renderer.planes.filter(
			(el) => el.type !== 'PingPongPlane' && el.target && el.uuid !== plane.uuid
		);
		...
		if (plane.target._depth) {
			...
			// findIndex
			siblingPlaneIndex = renderTargetsPlanes.findIndex(
				(el) => el.target.uuid === plane.target.uuid
			);
		}
	...
	addToRegularPlaneStack(plane) {
		...
		const planeStack = this.renderer.planes.filter(
			(el) =>
	...
	setPlaneRenderOrder(plane) {
		if (plane.type === 'ShaderPass') {
			this.sortShaderPassStack(
				plane._isScenePass ? this.stacks.scenePasses : this.stacks.renderPasses
			);
		...
		if (plane.target) {
			...
			const scenePassWithoutDepth = this.stacks.scenePasses.find(
				(pass, index) => pass._isScenePass && !pass._depth && index === 0
			);
