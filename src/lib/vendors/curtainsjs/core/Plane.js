...
export class Plane extends DOMMesh {
	...
	_documentToWorldSpace(vector) {
		return tempWorldPos2.set(
			((vector.x * this.renderer.pixelRatio) / this.renderer._boundingRect.width) *
				this._boundingRect.world.ratios.width,
			-((vector.y * this.renderer.pixelRatio) / this.renderer._boundingRect.height) *
	...
	_setWorldSizes() {
		...
		this._boundingRect.world = {
			width:
				((this._boundingRect.document.width / this.renderer._boundingRect.width) * ratios.width) /
				2,
			height:
				((this._boundingRect.document.height / this.renderer._boundingRect.height) *
					ratios.height) /
				2,
			ratios
		};
	...
	_setWorldPosition() {
		...
		this._boundingRect.world.top =
			((containerCenter.y - planeCenter.y) / this.renderer._boundingRect.height) *
			this._boundingRect.world.ratios.height;
		this._boundingRect.world.left =
			((planeCenter.x - containerCenter.x) / this.renderer._boundingRect.width) *
			this._boundingRect.world.ratios.width;
	}
	...
	setRelativeTranslation(translation) {
		if (!translation.type || translation.type !== 'Vec3') {
			if (!this.renderer.production) {
				throwWarning(
					this.type + ': Cannot set translation because the parameter passed is not of Vec3 type:',
					translation
				);
			}
	...
	mouseToPlaneCoords(mouseCoordinates) {
		...
		if (
			...
			const worldMouse = {
				x:
					2 *
						(mouseCoordinates.x / (this.renderer._boundingRect.width / this.renderer.pixelRatio)) -
					1,
				y:
					2 *
						(1 -
			...
			if (Math.abs(denominator) >= 0.0001) {
				const inverseViewMatrix = this._matrices.world.matrix
					.getInverse()
