...
export class Texture {
	...
	_setParent() {
		...
		if (this._parent._program) {
			if (!this._parent._program.compiled) {
				if (!this.renderer.production) {
					throwWarning(
						this.type + ': Unable to create the texture because the program is not valid'
					);
				}
			...
			this._size = {
				...
				height:
					(this._parent._size && this._parent._size.height) || this.renderer._boundingRect.height
			};
	...
	setSource(source) {
		...
		const sourceType =
			source.tagName.toUpperCase() === 'IMG' ? 'image' : source.tagName.toLowerCase();
		...
		if (this.sourceType === 'empty' || this.sourceType !== sourceType) {
			if (sourceType === 'video') {
				...
				if (!this.renderer.production) {
					throwWarning(
						this.type + ': this HTML tag could not be converted into a texture:',
						source.tagName
					);
				}
	...
	_updateGlobalTexParameters() {
		...
		if (this._globalParameters.floatingPoint === 'half-float') {
			if (this.renderer._isWebGL2 && this.renderer.extensions['EXT_color_buffer_float']) {
				...
				throwWarning(
					this.type + ': could not use half-float textures because the extension is not available'
				);
			}
			...
			if (this.renderer._isWebGL2 && this.renderer.extensions['EXT_color_buffer_float']) {
				...
				throwWarning(
					this.type + ': could not use float textures because the extension is not available'
				);
			}
		}
	}
